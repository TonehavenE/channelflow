...
#include <io.h>       // Replaces parts of unistd.h (read, write, close, etc.)
#include <process.h>  // Replaces parts of unistd.h (getpid, etc.)
#include <winsock2.h> // Replaces arpa/inet.h for htonl, ntohl, etc.

// Windows doesn't always have standard unistd.h constants, define if missing
#ifndef R_OK
#define R_OK 4
#define W_OK 2
#define X_OK 1
...
inline std::string removeSuffix(const std::string &filename,
...
inline bool hasSuffix(const std::string &filename,
                      const std::string &extension);
...
inline std::string ifstreamOpen(std::ifstream &is, const std::string &filebase,
...
inline Real L2Dist(const Eigen::VectorXd &x, const Eigen::VectorXd &y,
                   int cutoff = 0);
inline Real L2Dist(const Eigen::VectorXcd &x, const Eigen::VectorXcd &y,
                   int cutoff = 0);
inline Real L2Dist2(const Eigen::VectorXd &x, const Eigen::VectorXd &y,
                    int cutoff = 0);
inline Real L2Dist2(const Eigen::VectorXcd &x, const Eigen::VectorXcd &y,
                    int cutoff = 0);

inline Real L2IP(const Eigen::VectorXd &u, const Eigen::VectorXd &v,
                 int cutoff = 0);
...
inline void rescale(Eigen::VectorXd &x,
                    const Eigen::VectorXd &xscale); // x -> x./xscale
inline void unscale(Eigen::VectorXd &x,
                    const Eigen::VectorXd &xscale); // x -> x.*xscale

inline Real adjustDelta(Real delta, Real rescale, Real deltaMin, Real deltaMax,
                        std::ostream &os = std::cout);
inline Real adjustLambda(Real lambda, Real lambdaMin, Real lambdaMax,
                         std::ostream &os = std::cout);

inline void solve(const Eigen::MatrixXd &Ut, const Eigen::VectorXd &D,
                  const Eigen::MatrixXd &C, const Eigen::VectorXd &b,
                  Eigen::VectorXd &x);
...
// Neville algorithm quadratic inter/extrapolate based on f[0],f[1],f[2] and
// x[0],x[1],x[2]
inline Real quadraticInterpolate(const cfarray<Real> &fn,
                                 const cfarray<Real> &xn, Real x);
inline Real linearInterpolate(Real x0, Real f0, Real x1, Real f1, Real x);
inline bool isconst(cfarray<Real> f, Real eps = 1e-13);

// Neville algorithm (N-1)th order inter/extrapolate based on
// f[0],f[1],...,f[N-1] and x[0],x[1],...
inline Real polynomialInterpolate(const cfarray<Real> &fn,
                                  const cfarray<Real> &xn, Real x);

// secantSearch and bisectSearch:
// Find root of f(x) between a and b to tolerance feps from polynomial
// interpolant of {fn = f(xn)} I.e. rtn x st std::abs(f(x)) < feps and a <= x <=
// b.
inline Real secantSearch(Real a, Real b, cfarray<Real> &fn,
                         const cfarray<Real> &xn, Real feps = 1e-14,
                         int maxsteps = 50);

inline Real bisectSearch(Real a, Real b, cfarray<Real> &fn,
                         const cfarray<Real> &xn, Real feps = 1e-14,
                         int maxsteps = 50);
...
template <class T> inline void push(const T &t, cfarray<T> &cfarrayt);

inline void openfile(std::ofstream &f, std::string filename,
                     std::ios::openmode openflag = std::ios::out);

inline bool checkFlagContent(std::ifstream &is,
                             const std::vector<std::string> &flagList);
// ++++++++++++++++++++++++++++++++++++++++++++
// begin definitions
// ++++++++++++++++++++++++++++++++++++++++++++

inline void printout(const std::string &message, int taskid,
                     bool newline = true, std::ostream &os = std::cout) {
  if (taskid == 0) {
    os << message;
    if (newline)
      os << std::endl;
...
inline void printout(const std::string &message, bool newline = true,
                     std::ostream &os = std::cout) {
  int taskid = 0;
#ifdef HAVE_MPI
  int initialized = 0;
...
inline void printout(const std::string &message, std::ostream &os) {
  printout(message, true, os);
}

inline void printout(const std::stringstream &sstr,
                     std::ostream &os = std::cout) {
  printout(sstr.str(), false, os);
}

inline void printToFile(const std::string &message, const std::string &filename,
                        bool newline = true) {
  int taskid = 0;
#ifdef HAVE_MPI
  MPI_Comm_rank(MPI_COMM_WORLD, &taskid);
...
inline std::string appendSuffix(const std::string &filebase,
                                const std::string &extension) {
...
inline void write(std::ostream &os, int n) {
  if (sizeof(int) != 4) {
    std::cerr << "write(ostream& os, int n) :  channelflow binary IO assumes "
                 "32-bit int\n";
    std::cerr << "The int on this platform is " << 8 * sizeof(int) << " bits"
              << std::endl;
    exit(1);
  }
...
inline void read(std::istream &is, int &n) {
  if (sizeof(int) != 4) {
    std::cerr << "read(istream& is, int n) error: channelflow binary IO "
                 "assumes 32-bit int.\n";
    std::cerr << "The int on this platform is " << 8 * sizeof(int) << " bits"
              << std::endl;
    exit(1);
...
inline void write(std::ostream &os, Real x) {
  if (sizeof(double) != 8) {
    std::cerr << "write(ostream& os, Real x) error: channelflow binary IO "
                 "assumes 64-bit double-precision floating "
                 "point.\n";
    std::cerr << "The double on this platform is " << 8 * sizeof(Real)
              << " bits" << std::endl;
    exit(1);
  }
...
inline void read(std::istream &is, Real &x) {
  if (sizeof(double) != 8) {
    std::cerr << "read(istream& is, Real x) error: nsolver binary IO assumes "
  ...
  if (sizeof(int) != 4) {
    std::cerr << "write(ostream& os, Real x) error: nsolver binary IO assumes "
                 "64-bit double-precision floating point "
                 "and 32-bit ints.\n";
    std::cerr << "The int on this platform is " << 8 * sizeof(int) << " bits"
              << std::endl;
...
inline void load(Real &c, const std::string &filebase) {
  ...
  if (taskid == 0) {
    ...
    if (!is.good()) {
      std::cerr << "load(Real, filebase) :  can't open file " + filename
                << std::endl;
      exit(1);
    }
    Real r = 0;
...
inline Real L2Norm(const Eigen::VectorXd &x, int cutoff) {
  return sqrt(L2Norm2(x, cutoff));
}

inline Real L2Norm(const Eigen::VectorXcd &x, int cutoff) {
  return sqrt(L2Norm2(x, cutoff));
}
...
inline Real L2IP(const Eigen::VectorXd &u, const Eigen::VectorXd &v,
                 int cutoff) {
  if (u.size() != v.size()) {
    std::cerr << "error in L2IP(VectorXd, VectorXd) : vector length mismatch"
              << std::endl;
    exit(1);
  }
  Real sum = 0.0;
...
inline Real polynomialInterpolate(const cfarray<Real> &fn,
  ...
  if (N > Nbuffer) {
    cferror(
        "error in polynomial Interpolate(cfarray<Real> fn, cfarray<Real> xn, Real x) :\n\
            fn.length() == " +
        i2s(N) + " > " + i2s(Nbuffer) + " ==  static buffer size\n\
  ...
  for (int i = 1; i < N; i++) {
    ...
    int j_prev = (i - 1) % 2;
    for (int k = 0; k < (N - i); k++)
      y[j_curr][k] = linearInterpolate(xn[k], y[j_prev][k], xn[k + i],
                                       y[j_prev][k + 1], x);
  }
  return y[j_curr][0];
}
...
// Neville algorithm.
inline Real quadratic_extrapolate(const cfarray<Real> &f,
                                  const cfarray<Real> &mu, Real mug) {
  const int N = 3; // three points for quadratic extrapolation
  Real y[2][N];    // two cfarrays for quadratic extrap recurrence relation
  ...
  for (int i = 1; i < N; i++) {
    ...
    int j_prev = (i - 1) % 2;
    for (int k = 0; k < (N - i); k++)
      y[j_curr][k] = linear_extrapolate(mu[k], y[j_prev][k], mu[k + i],
                                        y[j_prev][k + 1], mug);
  }
  return y[j_curr][0];
}
...
inline Real adjustDelta(Real delta, Real deltaRate, Real deltaMin,
                        Real deltaMax, std::ostream &os) {
  ...
  // we try to go even lower, go ahead and set it lower, and the calling
  // function will check and return best current answer
...
inline Real adjustLambda(Real lambda, Real lambdaMin, Real lambdaMax,
                         std::ostream &os) {
  if (lambda < lambdaMin) {
    os << "lambda == " + r2s(lambda) + " is too small. Resetting to lambda == "
       << lambdaMin << std::endl;
    lambda = lambdaMin;
  } else if (lambda < lambdaMin) {
    os << "lambda == " + r2s(lambda) + " is too large. Resetting to lambda == "
       << lambdaMax << std::endl;
    lambda = lambdaMax;
  }
  return lambda;
...
inline void solve(const Eigen::MatrixXd &Ut, const Eigen::VectorXd &D,
                  const Eigen::MatrixXd &V, const Eigen::VectorXd &b,
                  Eigen::VectorXd &x) {
...
inline Real polyInterp(const cfarray<Real> &fa, const cfarray<Real> &xa,
                       Real x) {
  assert(fa.N() == xa.N());
  int N = fa.N();
  cfarray<Real> C(N);
  ...
  for (int m = 1; m < N; ++m) {
    ...
    fx += (df = (2 * (i_closest + 1) < (N - m) ? C[i_closest + 1]
                                               : D[i_closest--]));
  }
  return fx;
}
inline Real secantSearch(Real a, Real b, cfarray<Real> &fn,
                         const cfarray<Real> &xn, Real feps, int maxsteps) {
...
inline std::string stub(const std::string &filename, const std::string &ext) {
  ...
  std::string f = (s == std::string::npos)
                      ? filename
                      : filename.substr(s + 1, filename.length() - s);
...
template <class T> inline void push(const T &t, cfarray<T> &a) {
...
/**
 * @brief Sort the vector of eigenvalues passed in input by the abs of its
 * values in decreasing order and permute the matrix of eigenvectors
 * accordingly.
 *
...
inline void sort_by_abs(Vector &Lambda, Matrix &V) {
  ...
  std::sort(indexes.data(), indexes.data() + indexes.size(),
            [&Lambda](const size_t &ii, const size_t &jj) {
              return std::abs(Lambda(ii)) > std::abs(Lambda(jj));
            });
